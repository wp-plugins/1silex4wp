...
function arrayToXML($array)
{
	...
	$temp.="<?xml version=\"1.0\" encoding=\"UTF-8\"?><element type_str=\"object\" name_str=\"root\">$newline$tabs";
	$temp.=do_arrayToXML($array);
	$temp.="$newline$tabs</element>";
	return $temp;
}
...
//	for ($k=0;$k<$numElem;$k++)
	foreach ($array as $k => $value)
	{
		...
		if (is_string($value))
		{
			if ($value!="") $temp.="$newline$tabs<element type_str=\"string\" name_str=\"$k\">$newline$tabs<![CDATA[".$encoded_val."]]>$newline$tabs</element>";
		}
		else 
		{
			if(is_bool($value))
