...
this.setX=function(ua){ua=unescape(ua);this._x=ua;this.applyFrameProperties();}
this._y=0;this.getY=function(){return this._y;}
this.setY=function(va){va=unescape(va);this._y=va;this.applyFrameProperties();}
this._width=100;this.getWidth=function(){return this._width;}
this.setWidth=function(wa){wa=unescape(wa);this._width=wa;this.applyFrameProperties();}
this._height=100;this.getHeight=function(){return this._height;}
this.setHeight=function(xa){xa=unescape(xa);this._height=xa;this.applyFrameProperties();}
...
this.enterDom=function(){if(!this.frame){this.trace("enterDom "+this.idFrame+" - useIframe = "+this.getUseIframe());if(!$('#frameContainer').length)$('body').append('<div style="position: absolute; z-index: '+(this.nextZIndex++)+'" id="frameContainer" name="frameContainer"></div>');JsFrameBase.frameContainer=$('#frameContainer');var Aa;if(this.getUseIframe()==true)Aa=$('<iframe frameborder="0" style="position: absolute; z-index: '+(this.nextZIndex++)+'; background-color:transparent; border-width:0" id="'+this.idFrame+'" name="'+this.idFrame+'">');else Aa=$('<div style="overflow: auto; position: absolute; z-index: '+(this.nextZIndex++)+'; border: 0px; padding-top: 0px; padding-bottom: 0px; padding-left: 0px; padding-right: 0px;" id="'+this.idFrame+'" name="'+this.idFrame+'" >');$('#frameContainer').append(Aa);this.frame=Aa[0];var Ba=this;setTimeout(function(){Ba.applyFrameProperties();},1);}
